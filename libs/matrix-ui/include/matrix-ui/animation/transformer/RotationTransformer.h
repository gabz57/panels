...
//        progress %= angle_;
    ...
    virtual void MapPixel(int x, int y, int *matrix_x, int *matrix_y) {
        Rotate(x, y, DegToRad((float) progress), matrix_x, matrix_y);
    }
...
private:
    ...
    void Rotate(int x, int y, float angle_rad, int *new_x, int *new_y) {
        *new_x = (int) (((float) (x - cent_x)) * cosf(angle_rad) - (float) (y - cent_y) * sinf(angle_rad) + (float) cent_x);
        *new_y = (int) (((float) (x - cent_x)) * sinf(angle_rad) + (float) (y - cent_y) * cosf(angle_rad) + (float) cent_y);
    }
